...
M29CT adds an initial real FEAR v33 Model00p renderer: retained mesh/index/bone-weight data, CPU skinning from current node transforms, LOD0 drawing and client model skin/material tracking. The first hardware proof renders Model00p double-sided (`CullMode::None`) so D3D/Vita winding differences after CPU Y projection cannot hide otherwise valid weapon/prop/character meshes. The v113 world path remains deferred and bounded, with 8 MiB mesh + 16 MiB texture LRUs, 256px max world textures, invisible-material suppression, opaque/alpha-additive ordering and a 3000->7000 world triangle budget.

Vita controls are Killzone: Mercenary-oriented and routed through FEAR's own CBindMgr: left stick move, right stick look, L aim, R fire, X jump, Square reload, Triangle use, Circle crouch/sprint context, D-pad previous/next weapon + grenade + flashlight controls, front-touch SlowMo/melee/grenade selection, and alternate rear-touch sprint. The controller is polled once per retail frame so held state and press edges are consistent.

Release compile/link and full VPK packaging succeeded. `APP_VER=00.42`, title ID `FEAR00001`, same 22 package paths as 0.41. RX ends at `0x81EB3D3C` and RW begins at `0x81F00000`, with no overlap.
