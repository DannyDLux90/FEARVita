...
p=Path('src/platform/PlatformHomebrewMain.cpp'); s=p.read_text(encoding='utf-8')
old='#include "platform/PlatformVita.h"\n'; new=old+'#include "platform/vita_bootstrap.hpp"\n'
if old not in s: raise SystemExit('PlatformHomebrewMain include anchor not found')
...
#endif
'''
if old not in s: raise SystemExit('PlatformHomebrewMain main anchor not found')
s=s.replace(old,new,1)
...
new='''    PlatformManager::Get()->SystemInit();
    ...
    // Route SDL log output to a file before SDL_Init is called in VideoInit.
'''
if old not in s: raise SystemExit('PlatformHomebrewMain bootstrap anchor not found')
s=s.replace(old,new,1)
...
#endif
'''
if old not in s: raise SystemExit('PlatformHomebrewMain kfxmain anchor not found')
s=s.replace(old,new,1); p.write_text(s,encoding='utf-8')
p=Path('build/cmake/modules/PlatformVita.cmake'); s=p.read_text(encoding='utf-8')
old='ScePower_stub SceAudio_stub SceShaccCg_stub SceKernelDmacMgr_stub)'; new='ScePower_stub SceAudio_stub SceShaccCg_stub SceKernelDmacMgr_stub SceAppUtil_stub)'
if old not in s: raise SystemExit('PlatformVita.cmake stub anchor not found')
