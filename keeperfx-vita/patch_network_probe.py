...
# Source text contains two backslashes as CMake regex escaping; use a more
# stable insertion point if the exact literal differs.
if anchor not in s:
    anchor = '''if(NOT KEEPERFX_MATCHMAKING)\n    list(FILTER KEEPERFX_SOURCES_C EXCLUDE REGEX ".*/net_matchmaking\\.c$")\nendif()\n'''
...
# Gates: real networking must be ON and the single-player network stub must not
# be selected by CMake. Online-only pieces remain stubbed deliberately.
checks = {
