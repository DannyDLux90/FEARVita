...
static void mkdirs_for(const char *full_path, bool final_is_dir)
{
    ...
    for (size_t i = 1; i < len; ++i) {
        if (tmp[i] == '/') {
            tmp[i] = '\0';
            if (strchr(tmp, ':') != tmp + strlen(tmp) - 1) sceIoMkdir(tmp, 0777);
            tmp[i] = '/';
        }
    }
...
{
    if (g_apputil_initialized) return true;
    SceAppUtilInitParam init_param;
    SceAppUtilBootParam boot_param;
    memset(&init_param, 0, sizeof(init_param));
    memset(&boot_param, 0, sizeof(boot_param));
    int rc = sceAppUtilInit(&init_param, &boot_param);
    if (rc < 0) {
...
static bool deeper_requested()
{
    ...
    int rc = sceAppUtilReceiveAppEvent(&event_param);
    if (rc < 0 || event_param.type != SCE_APPUTIL_APPEVENT_TYPE_LIVEAREA) return false;
    int needed = sceAppUtilAppEventParseLiveArea(&event_param, NULL);
    if (needed <= 0 || needed > 256) return false;
    std::vector<char> buf((size_t)needed + 1, 0);
    rc = sceAppUtilAppEventParseLiveArea(&event_param, buf.data());
    if (rc < 0) return false;
    return strcmp(buf.data(), "-deeper") == 0;
}
...
static bool install_bundled_data()
{
    ...
    if (log) { fprintf(log, "KeeperFX Vita first-run data install begin\n"); fclose(log); }

    unzFile zf = unzOpen(kBundlePath);
    if (!zf) { log_line("unzOpen failed"); return false; }
    int rc = unzGoToFirstFile(zf);
    unsigned char *buffer = new (std::nothrow) unsigned char[256 * 1024];
    if (!buffer) { unzClose(zf); log_line("buffer alloc failed"); return false; }
    bool ok = true;
    ...
    while (rc == UNZ_OK) {
        ...
        if (unzGetCurrentFileInfo(zf, &info, name, sizeof(name) - 1, NULL, 0, NULL, 0) != UNZ_OK) { ok = false; break; }
        if (!safe_relative_name(name)) { log_line("unsafe path in data archive"); ok = false; break; }
        ...
        if (!is_dir) {
            ...
            if (existing != (long long)info.uncompressed_size) {
                if (unzOpenCurrentFile(zf) != UNZ_OK) { ok = false; break; }
                char tmp[800];
                snprintf(tmp, sizeof(tmp), "%s.kfxvita.tmp", dest);
                FILE *out = fopen(tmp, "wb");
                if (!out) { unzCloseCurrentFile(zf); ok = false; break; }
                for (;;) {
                    int got = unzReadCurrentFile(zf, buffer, 256 * 1024);
                    if (got < 0) { ok = false; break; }
                    if (got == 0) break;
                    if (fwrite(buffer, 1, (size_t)got, out) != (size_t)got) { ok = false; break; }
                    sceKernelPowerTick(SCE_KERNEL_POWER_TICK_DEFAULT);
                }
                fclose(out);
                int close_rc = unzCloseCurrentFile(zf);
                if (!ok || close_rc != UNZ_OK) { sceIoRemove(tmp); ok = false; break; }
                sceIoRemove(dest);
                if (sceIoRename(tmp, dest) < 0) { sceIoRemove(tmp); ok = false; break; }
            }
            ++files_done;
            if ((files_done & 0x7F) == 0) sceKernelPowerTick(SCE_KERNEL_POWER_TICK_DEFAULT);
        }
    ...
    if (ok && rc == UNZ_END_OF_LIST_OF_FILE) {
        FILE *m = fopen(kMarkerPath, "wb");
        if (!m) { log_line("could not write data marker"); return false; }
        fprintf(m, "KeeperFX Vita data v2\n");
        fclose(m);
...
static int run_kfxmain(int argc, char **argv, bool deeper)
{
    ...
    static char fallback0[] = "keeperfx";
    static char flag_campaign[] = "-campaign";
    static char campaign_name[] = "deepdngn";
    if (args.empty()) args.push_back(fallback0);
    args.push_back(flag_campaign);
    args.push_back(campaign_name);
    args.push_back(NULL);
    log_line("LiveArea direct start: Deeper Dungeons (campaign deepdngn)");
